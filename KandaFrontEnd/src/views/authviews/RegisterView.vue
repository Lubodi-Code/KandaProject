...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Registro exitoso, redirigir a la página de activación
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 429:
          // Demasiados intentos
          errors.value.general = 'Demasiados intentos de registro. Por favor, inténtalo más tarde.';
          break;
        case 500:
          // Error del servidor
          ...
          break;
        default:
          errors.value.general = 'Error al registrarse. Por favor, inténtalo de nuevo.';
      }
    ...
    <div class="max-w-md w-full space-y-8 bg-gray-900 p-8 rounded-xl shadow-2xl border border-indigo-500/30">
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="rounded-md -space-y-px">
          ...
          <div class="mb-4">
            <label for="password2" class="block text-sm font-medium text-gray-300 mb-1">Confirmar contraseña</label>
            <input id="password2" 
                   name="password2" 
                   type="password" 
                   autocomplete="new-password" 
                   ...
                   class="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-700 placeholder-gray-500 text-white bg-gray-800 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" 
                   placeholder="********" />
            <p v-if="errors.password2" class="mt-2 text-sm text-red-400">{{ errors.password2 }}</p>
          </div>
        </div>
